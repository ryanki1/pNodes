...
- **Test Mode Support**: Mock data generation for testing without live pNode connection
...
cd pNodes/xandeum-pnode-analytics
```
...
The Express proxy server is configured in `server/server.js`:
...
To connect to a different pNode, modify the `PNODE_ADDRESS` in `server/server.js`.

### Frontend Configuration

Frontend settings are in `src/app/services/constants.ts`:
...
| `npm run ng-serve` | Start Angular development server only |
...
1. Ensure the Express server is running (`npm run server`)
2. Check that port 3001 is available
3. Verify pNode address is correct in `server/server.js`
